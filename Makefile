...
install: clean
	...
	for content in $(PREFIX)usr/local/bin/* ; do \
		cd $(PREFIX)usr/local/bin/ && echo "############### :`echo $$content | grep -vc ^stalag13-` && if [ `echo $$content | grep -vc ^stalag13-` ]; then ln -fs /usr/local/bin/`basename $$content` $(PREFIX)usr/local/bin/`basename $$content | sed s/\\.[^.]*$$//g`; fi ; \
	done
