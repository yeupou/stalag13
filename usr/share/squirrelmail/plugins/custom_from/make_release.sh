...
# UPDATED VERSION AT
#         https://gitlab.com/yeupou/stalag13/raw/master/usr/share/squirrelmail/plugins/custom_from/make_release.sh
#
#                                 |     |
#                                 \_V_//
#                                 \/=|=\/
...
#
#
CONFIG_FILES=( config/config.php config/custom_from_access_table.php )
...
if [ ! -e docs/COPYING ]; then
   echo "No docs/COPYING file found.  Grabbing one from ../../doc"
   cp ../../doc/COPYING ./docs/
fi
if [ ! -e index.php ]; then
...
# custom_from has no template files
...
if [ ! -e config/index.php ]; then
   echo "No config/index.php file found.  Grabbing one from ../"
   cp ../index.php ./config/
fi
...
if [ ! -e config/.htaccess ]; then
   echo "No config/.htaccess file found.  Creating..."
